...
export class AuthService {
  ...
  constructor(
    ...
    this._loggedInSubject = new BehaviorSubject<boolean>(!!this._storage.tokens);
  ...
  signup(dto: SignupRequestDto): Observable<undefined> {
    return this._http.post<undefined>(this.JOIN_API_URL, dto);
  }
  ...
  logout(): Observable<undefined> {
    ...
    const observable$ = refreshToken
      ? this._http.get<undefined>(this.LOGOUT_API_URL, { headers })
      : of(undefined);

    return observable$.pipe(
      tap(() => {
        this._storage.tokens = null;
  ...
  getMe(): Observable<UserEntity | null> {
    const observable: Observable<UserEntity | null> = this.loggedIn
      ? this._http.get<UserEntity>(this.GET_MY_INFO_API_URL)
      : of(null);
